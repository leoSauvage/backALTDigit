        ...
        try {
            ...
            if (error instanceof Error) {
                return response.status(500).json({
                    error: error.message || 'Erreur lors de la création du modèle de contrat',
                })
            }
        }
    }
   ...
   */
    public async update({ params, request, response }: HttpContext) {
        try {
            ...
            const updatedTemplate = await ContractTemplateModel.updateTemplate(templateId, { name, templateContent })
    ...
    public async addField({ params, request, response }: HttpContext) {
        try {
            ...
            const field = await DynamicField.addDynamicField(templateId, { fieldId, x, y, page })
