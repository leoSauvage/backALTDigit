    ...
    try {
      ...
      if (!data.question || data.question < 3) {
        return response.status(422).json({
          errors: [
            {
              message: 'Le texte de la question est requis et doit comporter au moins 3 caractères',
            },
          ],
        })
      }
      ...
      if (!data.fieldKey) {
        return response.status(422).json({
          errors: [
            { message: 'La clé du champ est requise et doit comporter au moins 3 caractères' },
          ],
        })
      }
